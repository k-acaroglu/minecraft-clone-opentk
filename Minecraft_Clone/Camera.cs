...
namespace Minecraft_Clone
{
    internal class Camera
    {
        ...
        private float SENSITIVITY = 270f;
        ...
        private void UpdateVectors()
        {
            ...
            // This is all math, explanation under "other resources"
            front.X = MathF.Cos(MathHelper.DegreesToRadians(pitch)) * MathF.Cos(MathHelper.DegreesToRadians(yaw));
            front.Y = MathF.Sin(MathHelper.DegreesToRadians(pitch));
            front.Z = MathF.Cos(MathHelper.DegreesToRadians(pitch)) * MathF.Sin(MathHelper.DegreesToRadians(yaw));

            front = Vector3.Normalize(front); // scale a vector to normalized unit
            right = Vector3.Normalize(Vector3.Cross(front, Vector3.UnitY)); // Cross multiplication can be used to get the unkown axis
